...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    // 预检 WebGL2 能力：不可用则直接交给 .home-bg 的 CSS 渐变兜底，
    // 避免 three.js 在创建渲染器失败时刷一堆 console.error（易被误认为「出错了」）。
    const hasWebGL2 = (() => {
      try {
    ...
    const resize = () => {
      ...
      // 背景层基本不可见，1.5 已足够清晰，大幅降低 GPU 填充率
    ...
    let lastT = 0;
    let lastRender = 0;
    // 背景为缓慢辉光，30fps 已足够丝滑，可把 GPU 负载与耗电量砍掉约一半
    const TARGET_FPS = 30;
    ...
    const animate = (t) => {
      ...
      // 仅在达到目标帧间隔时渲染（动画时间仍按真实 dt 推进，保证流畅）
